...
COMMANDS:
  ...
  dump-ast <file>    Dump parsed AST (--output sexpr|json|pretty)
...
`);
    break;
  case "dump-ast": {
    // Delegate to the ast-dump.js module
    const dumpArgs = ["run", "--allow-read",
      new URL("./ast-dump.js", import.meta.url).pathname,
      ...Deno.args.slice(1)];
    const proc = new Deno.Command("deno", { args: dumpArgs });
